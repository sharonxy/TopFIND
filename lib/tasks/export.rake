...
desc "export cleavages and inhibitions to csv for cytoscape"
task :export_cleavage_network => :environment do
  
  require 'fastercsv'

  file = 'protein'

  path = '/Volumes/MMPfileserver/lab/Documents/Philipp/computational/topquant-xserv-production/public/analysis/'
  date = Date::today.to_s
  Dir.chdir(path)
  Dir.mkdir(date) unless  File.exists?(date)
  Dir.chdir(date)
     ...
     FasterCSV.open("#{file}-cleavageedgeattributes.csv", 'w',{:headers => true, :col_sep => "\t"}) do |csv|
       ...
       Protein.with_substrates.each do |p|
         p.cleavages.each do |c|
           if c.substrate.present? && (ntermids.include?(c.substrate.ac) || proteaseids.include?(c.substrate.id))
...
#  
     FasterCSV.open("#{file}-protein-cleavagepos.csv", 'w',{:headers => true, :col_sep => "\t"}) do |csv|
       csv << ['Protein','interaction','Proteinpos']
       Protein.with_substrates.each do |p|
         p.cleavages.each do |c|
#           
           if c.substrate.present? && (ntermids.include?(c.substrate.ac) || proteaseids.include?(c.substrate.id))
             csv << [c.substrate.ac,'position',"#{c.substrate.ac}-#{c.pos}"]
             csv << [p.ac,'cleavage',"#{c.substrate.ac}-#{c.pos}"]
           end
         end
       end
     end  
     ...
     FasterCSV.open("#{file}-nodeattributes.csv", 'w',{:headers => true, :col_sep => "\t"}) do |csv|
       csv << ['Protein','type','4T1overMam','4T1over67NR','merid','name']
       Protein.with_substrates.each do |p|
         csv<< [p.ac,'protease','','',p.merid,p.recname]
         p.cleavages.each do |c|
#           
           if c.substrate.present? && (ntermids.include?(c.substrate.ac) && !proteaseids.include?(c.substrate.id))
             csv << [c.substrate.ac,'protein','','','',c.substrate.recname]
           end
         end
       end
     end      
