...
jQuery.fn.tokenInput = function (url, options) {
...
jQuery.TokenList = function (input, settings) {
    ...
    var input_box = jQuery("<input type=\"text\">")
        .css({
            outline: "none"
        })
        .focus(function () {
    ...
    function init_list () {
        ...
        if(li_data && li_data.length) {
            for(var i in li_data) {
                var this_token = jQuery("<li><p>"+li_data[i].name+"</p> </li>")
                ...
                jQuery.data(this_token.get(0), "tokeninput", {"id": li_data[i].id, "name": li_data[i].name});
    ...
    function insert_token(id, value) {
      var this_token = jQuery("<li><p>"+ value +"</p> </li>")
      .addClass(settings.classes.token)
      .insertBefore(input_token);
      ...
      jQuery.data(this_token.get(0), "tokeninput", {"id": id, "name": value});
	...
	function highlight_term(value, term) {
		return value.replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + term + ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<b>jQuery1</b>");
	}
        ...
        if(results.length) {
            ...
            for(var i in results) {
                if (results.hasOwnProperty(i)) {
                    var this_li = jQuery("<li>"+highlight_term(results[i].name, query)+"</li>")
                    ...
                    jQuery.data(this_li.get(0), "tokeninput", {"id": results[i].id, "name": results[i].name});
                }
            }
    ...
    function run_search(query) {
        ...
        if(cached_results) {
            ...
            if(settings.method == "POST") {
			    jQuery.post(settings.url + queryStringDelimiter + settings.queryParam + "=" + query, {}, callback, settings.contentType);
		    } else {
		        jQuery.get(settings.url + queryStringDelimiter + settings.queryParam + "=" + query, {}, callback, settings.contentType);
		    }
        }
    }
};
