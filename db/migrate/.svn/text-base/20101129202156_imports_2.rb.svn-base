class Imports2 < ActiveRecord::Migration
  def self.up
    add_column :imports, :evidence_id, :integer
  ...
  def self.down
    remove_column :imports, :evidence_id

    add_index :evidencerelations, [:traceable_id], :name => 'index_evidencerelations_on_traceables_type_and_traceables_id'

    remove_index :imports, :name => :index_imports_on_evidence_id rescue ActiveRecord::StatementInvalid
