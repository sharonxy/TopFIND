class FixInhibition < ActiveRecord::Migration
  def self.up
    add_column :inhibitions, :inhibited_protease_id, :integer
    ...
    add_index :inhibitions, [:inhibited_protease_id]
  ...
  def self.down
    remove_column :inhibitions, :inhibited_protease_id

    add_index :evidencerelations, [:traceable_id], :name => 'index_evidencerelations_on_traceables_type_and_traceables_id'

    remove_index :inhibitions, :name => :index_inhibitions_on_inhibited_protease_id rescue ActiveRecord::StatementInvalid
